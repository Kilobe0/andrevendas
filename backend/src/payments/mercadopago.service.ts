...
export class MercadoPagoService {
  ...
  async createPreference(input: CreatePreferenceInput): Promise<{ id: string; initPoint: string }> {
    ...
    const publicApiUrl = process.env.PUBLIC_API_URL; // ex.: https://andrevendas-api.fly.dev
    const isHttps = (u?: string) => !!u && u.startsWith('https://');

    // Página de retorno do checkout. Em produção o site fica sob /andrevendas
    // (basePath do GitHub Pages), por isso é uma env própria — não dá para
    // derivar de FRONTEND_URL, que é só a origem usada no CORS.
    const returnUrl =
