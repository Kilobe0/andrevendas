...
const EVENT = 'av-admin-toast';
const FLASH_KEY = 'av_admin_toast';
...
export default function ToastHost() {
  ...
  useEffect(() => {
    ...
    // O layout do admin persiste entre navegações, então a mensagem "flash"
    // é conferida a cada troca de rota (dependência: pathname).
    const flash = sessionStorage.getItem(FLASH_KEY);
    if (flash) {
      sessionStorage.removeItem(FLASH_KEY);
      try { show(JSON.parse(flash)); } catch { /* mensagem corrompida: ignora */ }
    }
